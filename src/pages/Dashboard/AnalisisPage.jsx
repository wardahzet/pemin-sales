...
function AnalisisPage() {
  ...
  const distributorIdsFromSales = dummySalesData.map(
    (sale) => sale.distributor_id
  );
  ...
  const totalSalesPerMonthAndProduct = dummySalesData.reduce((acc, sale) => {
    ...
    const monthYearKey = `${saleDate.toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
    })} ${saleDate.getFullYear()}`;
    acc[monthYearKey] = acc[monthYearKey] || {};
    const product = dummyProductData.find(
  ...
  dummySalesData.forEach((sale) => {
    ...
    if (product) {
      ...
      totalStockPerProduct[productName] =
        (totalStockPerProduct[productName] || 0) + productStock;
    }
  });
  ...
  dummySalesData.forEach((sale) => {
    ...
    if (product && distributor) {
      const monthYearKey = new Date(sale.tanggal_pemesanan).toLocaleDateString(
        "en-US",
      ...
      if (monthYearKey === currentMonthYear) {
        ...
        distributionForCurrentMonth.push({
          distributorName,
          productName,
          quantity,
        });
  ...
  console.log(
    `Distribution for ${currentMonthYear}:`,
    distributionForCurrentMonth
  );

  return (
    <div className="flex w-full min-h-screen bg-[#E8EBF0] ">
      <Sidebar />
      <div className="w-10/12 mx-[20px] my-[27px] ">
        <div className="w-full h-[86px] bg-blue-500 rounded-t-lg mb-2 text-white font-medium text-[30px] px-5 flex items-center ">
          Analisis
        </div>
        <div className="flex grid-cols-2 gap-5">
          <div className="w-6/12">
            <div className="max-h-[240px] h-[240px] border-full border-blue-400 border-2 rounded-lg mb-3 py-2 bg-white">
              <div className="font-normal  text-xl text-center mb-2">
                Total penjualan produk per bulan
              </div>
              <div className="w-full pl-5">
                <BarChart
                  totalSalesPerMonthAndProduct={totalSalesPerMonthAndProduct}
                />
              </div>
            </div>
          ...
          <div className="w-6/12">
            <div className="max-h-[240px]">
              <div className="h-[110px] border-full border-blue-400  bg-white border-2 rounded-lg mb-4 flex justify-center items-center">
                <div className="font-bold text-[32px] mr-[42px]">
                  {totalSales}
                </div>
                <div className="font-normal text-xl mr-[42px]">
                  Jumlah Penjualan
                </div>
            ...
            <div className="h-[240px] border-full border-blue-400 border-2 rounded-lg mb-3 py-2 bg-white">
              <div className="font-normal text-xl text-center mb-2">
                Distribusi produk ke distributor per bulan
              </div>
              <div className="w-full flex justify-center">
